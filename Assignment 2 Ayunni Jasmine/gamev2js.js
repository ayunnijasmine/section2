...
window.rollDice()=>{
  ...
  ladders.forEach(ladder=>{
    if (ladder.start === currentPlayer.position) {
      console.log("you stepped on ladder!");
...
const players = [{name: "p1", position: 0, color: "pink"},
                 {name: "p2", position; 0, color: "cyan"}];
...
const ladders = [{start: 8, end: 15}, {start: 19, end: 24}, {start: 15, end: 5}, {start: 23, end; 16}];
...
function drawLine({color,startPos,endPos}){
  ...
  ctxt.beginPath();
  const sizeRatio = 1;
  ctxt.moveTo(startPos.x + 25,startPos.y + 25);
  ctxt.lineTo(endPos.x + 25,endPos.y + 25);
